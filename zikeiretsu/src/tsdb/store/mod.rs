pub mod read_only_store;
pub mod writable_store;

use crate::tsdb::{datapoint::*, search::*, timestamp_nano::*};
use async_trait::async_trait;
use std::cmp::Ordering;

use thiserror::Error;

#[derive(Error, Debug)]
pub enum StoreError {
    #[error("append error. {0}")]
    AppendError(String),

    #[error("unsorted datapoints. {0}")]
    UnsortedDatapoints(String),

    #[error("data field types mismatched. {0}")]
    DataFieldTypesMismatched(String),

    #[error("search error. {0}")]
    SearchError(String),
}

type Result<T> = std::result::Result<T, StoreError>;

pub(crate) fn check_datapoints_is_sorted(datapoints: &[DataPoint]) -> Result<()> {
    if datapoints.is_empty() {
        Ok(())
    } else {
        let mut prev = unsafe { datapoints.get_unchecked(0) };
        for each in datapoints[1..].iter() {
            if each.timestamp_nano.cmp(&prev.timestamp_nano) == Ordering::Less {
                return Err(StoreError::UnsortedDatapoints(format!(
                    "{:?}, {:?}",
                    each.timestamp_nano, prev.timestamp_nano
                )));
            }
            prev = each
        }

        Ok(())
    }
}

#[async_trait]
trait DatapointsStore {
    async fn datapoints(&mut self) -> Result<&[DataPoint]>;
    async fn search(&mut self, cond: DatapointSearchCondition) -> Option<&[DataPoint]> {
        let datapoints = match self.datapoints().await {
            Ok(datapoints) => datapoints,
            Err(_) => return None,
        };

        DataPoint::search(datapoints, cond).await
    }
}

#[cfg(test)]
mod test {
    use super::*;
    use crate::tsdb::store::DataPoint;
    use async_trait::async_trait;

    macro_rules! empty_data_points {
        ($($timestamp:expr),*) => {
            vec![
            $(DataPoint::new(TimestampNano::new($timestamp),vec![])),*
            ]
        };
    }

    macro_rules! ts {
        ($v:expr) => {
            TimestampNano::new($v)
        };
    }

    struct SearchTestStore {
        datas: Vec<DataPoint>,
    }

    #[async_trait]
    impl DatapointsStore for SearchTestStore {
        async fn datapoints(&mut self) -> Result<&[DataPoint]> {
            Ok(self.datas.as_slice())
        }
    }

    #[tokio::test]
    async fn binsearch_test_1() {
        let datas = empty_data_points!(9, 10, 19, 20, 20, 20, 30, 40, 50, 50, 51);
        let mut store = SearchTestStore { datas };
        let result = store
            .search(DatapointSearchCondition::since(ts!(20)).with_until(ts!(50)))
            .await;
        assert!(result.is_some());
        let result = result.unwrap();

        assert_eq!(result, empty_data_points!(20, 20, 20, 30, 40, 50, 50));
    }

    #[tokio::test]
    async fn binsearch_test_2() {
        let datas = empty_data_points!(9, 10, 19, 20, 20, 20, 30, 40, 50, 50, 51);
        let mut store = SearchTestStore { datas };
        let result = store.search(DatapointSearchCondition::since(ts!(20))).await;
        assert!(result.is_some());
        let result = result.unwrap();

        assert_eq!(result, empty_data_points!(20, 20, 20, 30, 40, 50, 50, 51));
    }

    #[tokio::test]
    async fn binsearch_test_3() {
        let datas = empty_data_points!(9, 10, 19, 20, 20, 20, 30, 40, 50, 50, 51);
        let mut store = SearchTestStore { datas };
        let result = store.search(DatapointSearchCondition::until(ts!(40))).await;
        assert!(result.is_some());
        let result = result.unwrap();

        assert_eq!(result, empty_data_points!(9, 10, 19, 20, 20, 20, 30, 40));
    }
}
