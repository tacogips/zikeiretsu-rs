...
use super::dataseries_ref::ArrowSeries;
use arrow::array::{
    Array, ArrayData, ArrayRef, BooleanArray, Field, Float64Array, Int32Array, Int32Builder,
    Int64Array, ListArray, NullArray, PrimitiveArray, StringArray, StructArray,
    TimestampNanosecondArray, TimestampSecondArray, UInt64Array,
};
use arrow::buffer::Buffer;
use arrow::datatypes::{DataType, Date64Type, Field, Schema, Time64NanosecondType, ToByteSlice};
    ...
    async fn as_arrow_dataframes(&self, timezone: Option<&FixedOffset>) -> Result<RecordBatch> {
        ...
        let arrays = field_names
            ...
            .map(|(field_name, each_series)| {
                each_series
                    .as_arrow_field(timezone)
                    .and_then(|array| match array {
                        ArrowSeries::UInt64(arr) => (
                            Field::new(field_name, DataType::UInt64, false),
                            Arc::new(arr),
                        ),
                        ArrowSeries::Float64(arr) => {
                            unimplemented!()
                        }
                        ArrowSeries::Bool(arr) => {
                            unimplemented!()
                        }
                        ArrowSeries::String(arr) => {
                            unimplemented!()
                        }
                        ArrowSeries::TimestampNano(arr) => {
                            unimplemented!()
                        }
                        ArrowSeries::TimestampSec(arr) => {
                            unimplemented!()
                        }
                        ArrowSeries::Vacant(arr) => {
                            unimplemented!()
                        }
                    })
            })
